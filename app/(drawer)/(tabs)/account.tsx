...
export default function Profile() {
  ...
  // ✅ Real Track Order handler
  const handleTrackOrder = async () => {
    const lastOrderId = await AsyncStorage.getItem("@last_order_id");

    if (!lastOrderId) {
      Alert.alert(
        "No active order",
        "You don’t have any recent order to track.\n\nPlace an order first, then come back here.",
        [{ text: "OK" }]
      );
      return;
    }

    router.push(`/order_tracking/${lastOrderId}` as any);
  };
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <TouchableOpacity style={styles.logoutButton} onPress={handleLogout}>
        ...
        <Text style={styles.logoutText}>{isGuest ? "Exit Guest Mode" : "Log Out of Account"}</Text>
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  guestInfo: { flexDirection: "column" },
  welcomeText: { fontSize: 14, color: "#666" },
  guestText: { fontSize: 20, fontWeight: "bold", color: "#E53935", marginTop: 2 },
  ...
  bannerTitle: { fontSize: 16, fontWeight: "600", color: "#D97706", marginBottom: 4 },
  bannerSubtitle: { fontSize: 13, color: "#92400E" },

  nameRow: { flexDirection: "row", alignItems: "center" },
  name: { fontSize: 20, fontWeight: "bold", marginRight: 8, marginLeft: 2 },
  email: { fontSize: 16, fontWeight: "600", marginLeft: 2, marginTop: 4 },
  phone: { fontSize: 16, fontWeight: "600", color: "gray", marginLeft: 2, marginTop: 2 },

  options: { borderTopWidth: 1, borderColor: "#ddd", marginBottom: 20, marginTop: 10 },
  optionRow: {
    flexDirection: "row",
